...
module.exports = class BotUsers {
    ...
    getUsers(err, res, done) {
        ...
        if (!(this.nUsers % 100)) {
            ...
            var uniqueUsers = _users.filter(function (item, pos) {
                return _users.indexOf(item) == pos;
            })
            jsonfile.writeFile(this.database, uniqueUsers, function (err) {
                console.log("End");
                _this.users = [];
            })
        }
